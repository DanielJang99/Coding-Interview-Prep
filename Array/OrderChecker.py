...
def is_first_come_first_served(take_out_orders, dine_in_orders, served_orders):
    ...
    if (len(dine_in_orders) + len(take_out_orders) != len(served_orders)):
        return False

    for i in range(len(served_orders)):
        order = served_orders[i]
        if order in take_out_orders:
            if take_out_orders.index(order) < take_out_cnt:
                return False
            else:
                take_out_cnt = take_out_orders.index(order)
        elif order in dine_in_orders:
            if dine_in_orders.index(order) < dine_in_cnt:
                return False
            else:
                dine_in_cnt = dine_in_orders.index(order)
        else:
            return False
    return True
